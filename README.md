...
- Episodes are grouped into sections (e.g. Beginner/Advanced); the panel shows sections first, each with its own progress bar, and opens into that section's episode list.
...
- The video player is fully custom-built (no browser default controls), with Previous/Next, quick emoji reactions, and a chat panel — messages also pop up briefly over the video itself.
...
From the left sidebar, go to **SQL Editor**, click "New query", and run this for shared watch progress:
...
These are the only two tables the site needs. (An earlier version of this project also used a `rooms` table for room codes — that concept doesn't exist anymore, so if you have a leftover `rooms` table from before, it's safe to ignore or delete.)
...
1. Open the site, enter your name, pick a section, then an episode from its list, and click "Start".
...
3. Inside the watch page, "Previous"/"Next" (in the player's control bar) moves *you* to the next episode, without affecting your friend if they're somewhere else — just like switching voice channels.
4. Use the chat panel below the video, or the quick emoji reactions in the player, to react in real time — chat messages also pop up briefly over the video itself.
